...
func Merchant(perso *Character, monster *Monster) { // Afficher les options de l'interface merchant
	...
	fmt.Println("3- Acheter une fourrure de loup (Coût : 4 pièces d'or)")
	fmt.Println("4- Acheter une peau de troll (Coût : 7 pièces d'or)")
	fmt.Println("5- Acheter du cuir de sanglier (Coût : 3 pièces d'or)")
	fmt.Println("6- Acheter une plume de corbeau (Coût : 1 pièce d'or)")
	fmt.Println("7- Acheter un livre de sort : boule de feu (Coût : 25 pièces d'or)")
	fmt.Println("8- Acheter un livre de sort : éclair de givre (Coût : 40 pièces d'or)")
	fmt.Println("9- Acheter 10 emplacements de sac (Coût : 30 pièce d'or)")
	fmt.Println("0- Revenir dans le menu principal")
	fmt.Scan(&menumerchant)
	for menumerchant < 0 || menumerchant > 9 { // Si l'utilisateur entre un caractère plus petit ou plus grand, une erreur est renvoyée
